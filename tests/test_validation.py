...
class TestInputValidator:
    ...
    def test_validate_number_for_base(self):
        ...
        # Prefix mismatch
        with pytest.raises(ValidationError, match="Prefix '0B' indicates base 2, but base 10 specified"):
            self.validator.validate_number_for_base("0b1010", 10)
            
        with pytest.raises(ValidationError, match="Prefix '0X' indicates base 16, but base 10 specified"):
            self.validator.validate_number_for_base("0xFF", 10)
            
        # Empty after prefix removal
        with pytest.raises(ValidationError, match="Number cannot be empty after removing prefix"):
    ...
    def test_suggestion_accuracy(self):
        ...
        for number, expected_base in test_cases:
            ...
            try:
                ...
                pytest.fail(f"Suggested base {suggested} is not valid for number {number}")
