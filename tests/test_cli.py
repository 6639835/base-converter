...
class TestBaseConverterCLI:
    ...
    def test_run_interactive_mode_flag(self):
        ...
        with patch.object(self.cli, 'interactive_mode', return_value=None) as mock_interactive:
    ...
    def test_error_handling(self):
        ...
        exit_code = self.cli.run(['G', '-f', '10', '-t', '2'])  # Invalid digit for decimal
        assert exit_code == 1
        
        # Test with invalid base
        exit_code = self.cli.run(['123', '-f', '37', '-t', '2'])  # Base 37 not supported
        assert exit_code == 1
