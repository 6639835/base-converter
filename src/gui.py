...
class BaseConverterGUI:
    ...
    def create_menu_bar(self):
        ...
        tools_menu.add_command(label="Base Information", command=self.show_base_info_dialog)
        tools_menu.add_command(label="Arithmetic Calculator", command=self.show_arithmetic_dialog)
    ...
    def create_input_section(self, parent):
        ...
        # Number input
        ttk.Label(input_frame, text="Number:").grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        self.number_var = tk.StringVar()
        self.number_entry = ttk.Entry(input_frame, textvariable=self.number_var, font=('Consolas', 11))
        self.number_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(0, 5))
        
        # Detect base button
        self.detect_button = ttk.Button(input_frame, text="Detect Base", command=self.detect_base)
        self.detect_button.grid(row=0, column=2, padx=(5, 0))
        
        # Source base
        ttk.Label(input_frame, text="From Base:").grid(row=1, column=0, sticky=tk.W, pady=(10, 0), padx=(0, 5))
        self.from_base_var = tk.StringVar(value="10")
        self.from_base_combo = ttk.Combobox(input_frame, textvariable=self.from_base_var, width=15)
        self.from_base_combo['values'] = [str(i) for i in range(2, 37)]
        self.from_base_combo.grid(row=1, column=1, sticky=tk.W, pady=(10, 0), padx=(0, 5))
        
        # Common base buttons for input
        base_frame = ttk.Frame(input_frame)
        base_frame.grid(row=1, column=2, pady=(10, 0), padx=(5, 0))
        
        ttk.Button(base_frame, text="Bin", width=5, command=lambda: self.set_base('from', 2)).grid(row=0, column=0, padx=1)
        ttk.Button(base_frame, text="Oct", width=5, command=lambda: self.set_base('from', 8)).grid(row=0, column=1, padx=1)
        ttk.Button(base_frame, text="Dec", width=5, command=lambda: self.set_base('from', 10)).grid(row=0, column=2, padx=1)
        ttk.Button(base_frame, text="Hex", width=5, command=lambda: self.set_base('from', 16)).grid(row=0, column=3, padx=1)
    ...
    def create_conversion_section(self, parent):
        ...
        convert_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        convert_frame.columnconfigure(1, weight=1)
        
        # Target base
        ttk.Label(convert_frame, text="To Base:").grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        self.to_base_var = tk.StringVar(value="2")
        self.to_base_combo = ttk.Combobox(convert_frame, textvariable=self.to_base_var, width=15)
        self.to_base_combo['values'] = [str(i) for i in range(2, 37)]
        self.to_base_combo.grid(row=0, column=1, sticky=tk.W, padx=(0, 5))
        
        # Convert button
        self.convert_button = ttk.Button(convert_frame, text="Convert", command=self.convert_number)
        ...
        ttk.Button(out_base_frame, text="Binary", command=lambda: self.quick_convert(2)).grid(row=0, column=0, padx=2)
        ttk.Button(out_base_frame, text="Octal", command=lambda: self.quick_convert(8)).grid(row=0, column=1, padx=2)
        ttk.Button(out_base_frame, text="Decimal", command=lambda: self.quick_convert(10)).grid(row=0, column=2, padx=2)
        ttk.Button(out_base_frame, text="Hexadecimal", command=lambda: self.quick_convert(16)).grid(row=0, column=3, padx=2)
        ttk.Button(out_base_frame, text="Show Table", command=self.show_conversion_table).grid(row=0, column=4, padx=2)
    ...
    def create_results_section(self, parent):
        ...
        result_label = ttk.Label(results_frame, textvariable=self.result_var, font=('Consolas', 14, 'bold'),
                               foreground='darkgreen', background='lightgray', padding="5")
        ...
        self.history_text = scrolledtext.ScrolledText(results_frame, height=8, width=70, 
    ...
    def create_tools_section(self, parent):
        ...
        tools_frame.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 5))
        ...
        ttk.Checkbutton(tools_frame, text="Add prefix (0x, 0b, 0o)", 
                       variable=self.add_prefix_var).grid(row=0, column=0, sticky=tk.W, padx=(0, 10))
        ttk.Checkbutton(tools_frame, text="Format with separators", 
                       variable=self.format_separators_var).grid(row=0, column=1, sticky=tk.W, padx=(0, 10))
        ttk.Checkbutton(tools_frame, text="Uppercase letters", 
                       variable=self.uppercase_var).grid(row=0, column=2, sticky=tk.W)
    ...
    def convert_number(self):
        ...
        try:
            ...
            # Validate input
            validated_number, _ = self.validator.validate_number_for_base(number, from_base)
            ...
            # Add to history
            history_entry = f"{number} (base {from_base}) → {formatted_result} (base {to_base})"
    ...
    def show_conversion_table(self):
        ...
        try:
            ...
            ttk.Label(frame, text=f"Conversion table for '{number}' (base {from_base})", 
            ...
            ttk.Label(headers_frame, text="Base", font=('Arial', 10, 'bold'), width=8).pack(side=tk.LEFT)
            ttk.Label(headers_frame, text="Name", font=('Arial', 10, 'bold'), width=15).pack(side=tk.LEFT)
            ttk.Label(headers_frame, text="Value", font=('Arial', 10, 'bold')).pack(side=tk.LEFT)
            ...
            for base in [2, 8, 10, 16]:
                ...
                ttk.Label(row_frame, text=str(base), width=8, font=('Consolas', 9)).pack(side=tk.LEFT)
                ttk.Label(row_frame, text=base_names[base], width=15, font=('Arial', 9)).pack(side=tk.LEFT)
                ttk.Label(row_frame, text=table[base], font=('Consolas', 9, 'bold')).pack(side=tk.LEFT)
    ...
    def show_base_info_dialog(self):
        ...
        ttk.Label(frame, text="Supported Number Bases", font=('Arial', 14, 'bold')).pack(pady=(0, 10))
        
        # Create scrollable text widget
        text_widget = scrolledtext.ScrolledText(frame, height=20, width=60, font=('Consolas', 9))
        ...
        for base in common_bases:
            ...
            info_text += f"  Digits: {', '.join(info['digits'][:min(10, len(info['digits']))])}"
    ...
    def show_arithmetic_dialog(self):
        ...
        # Input fields
        ttk.Label(frame, text="First Number:").grid(row=0, column=0, sticky=tk.W, pady=5)
        num1_var = tk.StringVar()
        ttk.Entry(frame, textvariable=num1_var, width=20).grid(row=0, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        
        ttk.Label(frame, text="Second Number:").grid(row=1, column=0, sticky=tk.W, pady=5)
        num2_var = tk.StringVar()
        ttk.Entry(frame, textvariable=num2_var, width=20).grid(row=1, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        
        ttk.Label(frame, text="Base:").grid(row=2, column=0, sticky=tk.W, pady=5)
        base_var = tk.StringVar(value="10")
        ttk.Combobox(frame, textvariable=base_var, values=[str(i) for i in range(2, 37)], width=10).grid(row=2, column=1, sticky=tk.W, pady=5)
        
        ttk.Label(frame, text="Operation:").grid(row=3, column=0, sticky=tk.W, pady=5)
        op_var = tk.StringVar(value="+")
        ttk.Combobox(frame, textvariable=op_var, values=["+", "-", "*", "/", "%", "**"], width=10).grid(row=3, column=1, sticky=tk.W, pady=5)
        
        # Result
        ttk.Label(frame, text="Result:").grid(row=4, column=0, sticky=tk.W, pady=5)
        result_var = tk.StringVar()
        ttk.Label(frame, textvariable=result_var, font=('Consolas', 10, 'bold'), foreground='darkgreen').grid(row=4, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        ...
        ttk.Button(frame, text="Calculate", command=calculate).grid(row=5, column=1, pady=10)
    ...
    def show_batch_dialog(self, filename: Optional[str] = None):
        ...
        ttk.Label(frame, text="Batch Conversion", font=('Arial', 12, 'bold')).pack(pady=(0, 10))
        
        # Input area
        input_frame = ttk.LabelFrame(frame, text="Numbers (one per line)", padding="5")
        input_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        input_text = scrolledtext.ScrolledText(input_frame, height=10, width=70, font=('Consolas', 9))
        ...
        ttk.Combobox(control_frame, textvariable=from_base_var, values=[str(i) for i in range(2, 37)], width=5).pack(side=tk.LEFT, padx=(0, 10))
        
        ttk.Label(control_frame, text="To Base:").pack(side=tk.LEFT, padx=(0, 5))
        to_base_var = tk.StringVar(value="2")
        ttk.Combobox(control_frame, textvariable=to_base_var, values=[str(i) for i in range(2, 37)], width=5).pack(side=tk.LEFT, padx=(0, 10))
        
        def process_batch():
            try:
                numbers = [line.strip() for line in input_text.get("1.0", tk.END).splitlines() if line.strip()]
                if not numbers:
                ...
                result_text = scrolledtext.ScrolledText(result_frame, font=('Consolas', 9))
        ...
        ttk.Button(control_frame, text="Convert All", command=process_batch).pack(side=tk.RIGHT)
