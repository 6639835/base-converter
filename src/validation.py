...
class InputValidator:
    ...
    def validate_base(self, base: Union[str, int]) -> int:
        ...
        if base_int < self.min_base or base_int > self.max_base:
            raise ValidationError(f"Base must be between {self.min_base} and {self.max_base}, got {base_int}")
    ...
    def validate_number_string(self, number: str) -> str:
        ...
        if not isinstance(number, str):
            raise ValidationError(f"Number must be a string, got {type(number).__name__}")
        ...
        if len(cleaned) > self.max_input_length:
            raise ValidationError(f"Number too long (max {self.max_input_length} characters)")
    ...
    def validate_number_for_base(self, number: str, base: int) -> Tuple[str, bool]:
        ...
        if cleaned_number.upper().startswith(('0B', '0O', '0X')):
            ...
            if expected_base and expected_base != base:
                raise ValidationError(f"Prefix '{prefix}' indicates base {expected_base}, but base {base} specified")
        ...
        for i, digit in enumerate(cleaned_number.upper()):
            if digit not in valid_digits:
                raise ValidationError(f"Invalid digit '{digit}' for base {base} at position {i}")
    ...
    def validate_operation(self, operation: str) -> str:
        ...
        if operation not in valid_operations:
            raise ValidationError(f"Invalid operation '{operation}'. Valid operations: {', '.join(valid_operations)}")
